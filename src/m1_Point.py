...
# D (continued)  in file  m0_INSTRUCTIONS.txt, asking questions as needed.
#
# DO (continued): Then implement a class called   Point that has NO METHODS
# DO (continued)  yet, just the lines that start the definition of any class:
#
...
class Point(object):
    ...
    def get_number_of_moves_made(self):
        total = 0
        if self.move_to(self.x, self.y) != self:
            total = total + 1
        if self.move_by(self.x, self.y) == self:
            total = total + 1
        return total
    ...
    def closer_to(self, p1, p2):
        xd1 = p1.x - self.x
        yd1 = p1.y - self.y
        d1 = math.sqrt(xd1 ** 2 + yd1 ** 2)
        xd2 = p2.x - self.x
        yd2 = p2.y - self.y
        d2 = math.sqrt(xd2 ** 2 + yd2 ** 2)
        if d1 > d2:
            return p2
        else:
            return p1
    ...
    def halfway_to(self, p):
        x = self.x + (p.x - self.x)/2
        y = self.y + (p.y - self.y)/2
        return Point(x, y)
